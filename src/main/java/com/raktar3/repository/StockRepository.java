...
public interface StockRepository extends CrudRepository<Stock, Integer> {
	...
	@Query(value="select sum(amount) from stock where beerkezes=true", nativeQuery=true)
	int findToSelejt();
	
	@Query(value = "select distinct product_id from stock", nativeQuery = true)
	List<Integer> findDistinctProduct();
	
//	@Query(value="select sum(amount) from stock where product_id=?1 and beerkezes=true", nativeQuery=true)
//	Integer getAmountByProduct(Integer id);    // BEJÖVŐ
	
	@Query(value="select sum(amount) as BE from aquaraktar.stock where product_id=?1 and beerkezes=true", nativeQuery=true)
	Integer getAmountByProduct(Integer id);    // BEJÖVŐ


	@Query(value="select sum(amount) from stock where product_id=?1 and (eladas=true or selejt=true)", nativeQuery=true)
	...
	public void deleteSelectedRowsProduct(Integer id);
